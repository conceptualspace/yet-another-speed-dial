...
function insertCachedBookmark(parentId, node, nextSiblingId) {
    ...
    folder.children = folder.children || [];
    let position;
    if (nextSiblingId && nextSiblingId !== -1) {
        const siblingPosition = folder.children.findIndex(child => child.id === nextSiblingId);
        if (siblingPosition === -1) return false;
        position = siblingPosition + (settings.defaultSort === "first" ? 1 : 0);
    } else {
        position = settings.defaultSort === "first" ? 0 : folder.children.length;
    }

    node.parentId = parentId;
    folder.children.splice(position, 0, node);
    folder.children.forEach((child, index) => child.index = index);
    return true;
}
...
async function openAllTabs(folderId = currentFolder, groupTabs = false) {
    const targetId = folderId || speedDialId;
    let urls;

    // only the active folder is kept in the dom, so fall back to the bookmark tree
    let folder = document.getElementById(targetId);
    if (folder) {
        urls = [...folder.getElementsByClassName('tile')].map(dial => dial.href).filter(Boolean);
    } else {
        const node = targetId === speedDialId ? speedDialRootNode : folderNodeMap.get(targetId);
        urls = (node?.children || []).map(child => child.url).filter(Boolean);
    }
            ...
            if (evt.clone.dataset.type === 'folder') {
                ...
                if (movedBetweenTabContainers) {
                    ...
                    if (movedNode && insertCachedBookmark(toParentId, movedNode, newSiblingId)) {
                        ...
                        appendCachedBookmark(fromParentId, movedNode);
                    }
                }
...
// a title/url edit touches one dial, so patch it in place. rebuilding every folder
// blanks all the thumbnails until they reload, and costs more the more dials exist
function applyBookmarkChange(change) {
    ...
    if (change.isFolder) {
        ...
        foldersContainer.querySelectorAll(`[folderid="${change.id}"]`)
            .forEach(element => element.textContent = change.title);
        const folderDialTitle = bookmarksContainerParent
            .querySelector(`.folderDial[data-id="${change.id}"] .folderDial-titleText`);
        if (folderDialTitle) folderDialTitle.textContent = change.title;
        return;
    }

    const cachedNode = getCachedFolderNode(change.parentId)?.children
        ?.find(child => child.id === change.id);
    if (cachedNode) {
        cachedNode.title = change.title;
        cachedNode.url = change.url;
    }
    ...
    const tile = bookmarksContainerParent.querySelector(`.tile[data-id="${change.id}"]`);
    if (!tile) return;

    const urlChanged = tile.href !== change.url;
    tile.href = change.url;
    const title = tile.querySelector('.tile-title');
    if (title) title.textContent = change.title;

    if (!change.thumbnail && urlChanged) {
        // the new site has no image yet; it arrives as a thumbBatch once captured
...
function handleMessages(message) {
    ...
    if (message.data?.refresh) {
        ...
        // without a thumbnail the dial is still being captured, so leave the toast up
        if (message.data?.thumbnail) hideToast();
