...
async function printNewSetup() {
    ...
    // Feature rows re-use the localized newInstall strings. Some locales prefix
    // these with emoji, so strip any leading pictographs and pair each line with
    // a matching Material icon instead.
    const featureIcons = [
        // add bookmarks (star)
        ...
        '<svg class="welcome-feature-icon" xmlns="http://www.w3.org/2000/svg" height="22" viewBox="0 -960 960 960" width="22" fill="currentColor"><path d="M160-160q-33 0-56.5-23.5T80-240v-480q0-33 23.5-56.5T160-800h240l80 80h320q33 0 56.5 23.5T880-640v400q0 33-23.5 56.5T800-160H160Z"/></svg>',
        // edit / rearrange
    ...
    const featureRows = ['newInstall2', 'newInstall3', 'newInstall4']
        .map((key, i) => {
            const raw = chrome.i18n.getMessage(key);
            if (!raw) return '';
            const text = raw.replace(/^[\p{Extended_Pictographic}\u{FE0F}\u{200D}\s]+/u, '').trim();
            return `<li class="welcome-feature">${featureIcons[i]}<span data-locale="${key}">${text}</span></li>`;
        })
        .filter(Boolean)
        .join('');
